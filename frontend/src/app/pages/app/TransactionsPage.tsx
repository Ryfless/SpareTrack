              ...
              {filtered.length === 0
                ...
                : filtered.map(trx => {
                    ...
                    return <tr key={trx.id} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                      <td className="px-4 py-3 text-xs text-slate-400" style={{ fontFamily:"'JetBrains Mono', monospace" }}>{trx.id}</td>
                      <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400">{trx.date}</td>
                      ...
                      <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">{trx.sparepart}</td>
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{trx.branch}</td>
                      <td className="px-4 py-3 text-center"><span className={`font-semibold text-sm ${trx.type==="in"?"text-emerald-600":trx.type==="out"?"text-red-600":"text-slate-600"}`} style={{ fontFamily:"'JetBrains Mono', monospace" }}>{trx.type==="in"?"+":trx.type==="out"?"−":""}{Math.abs(trx.qty)}</span></td>
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{trx.by}</td>
                    </tr>;
                  })
              }
            </tbody>
