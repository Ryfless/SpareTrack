...
import { ArrowUpRight, Info, Loader2, Lock } from "lucide-react";
...
export function StokKeluarModal({ open, onClose, sparepart, userProfile, currentRole }: { open: boolean; onClose: () => void; sparepart?: SparepartDetail; userProfile?: { role: string; branch: string } | null; currentRole?: string }) {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      await createTransaction({ type: 'out', sparepart_id: form.sparepart_id, branch_id: form.branch_id, quantity: Number(form.jumlah), notes: form.catatan });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Catat Stok Keluar" size="sm">
      ...
      <div className="flex gap-3 mt-5">
        <button onClick={submit} disabled={submitting} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 active:scale-95 rounded-xl transition-all disabled:opacity-70">
          {submitting ? <><Loader2 size={15} className="animate-spin" />Menyimpan...</> : <><ArrowUpRight size={15} />Simpan</>}
        </button>
