...
export function StokKeluarModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [form, setForm] = useState({ sparepart: "", cabang: "", jumlah: "", alasan: "", catatan: "" });
  const set = (k: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => setForm(f => ({ ...f, [k]: e.target.value }));
  return (
    <Modal open={open} onClose={onClose} title="Catat Stok Keluar" size="sm">
      <div className="space-y-4">
        ...
        <FormField label="Alasan"><select value={form.alasan} onChange={set("alasan")} className={inputCls}><option value="">Pilih alasan</option><option>Servis Kendaraan</option><option>Penjualan Eceran</option><option>Retur ke Supplier</option><option>Rusak/Tidak Layak</option><option>Lainnya</option></select></FormField>
      ...
      <div className="flex gap-3 mt-5">
        <button onClick={() => { if (!form.sparepart || !form.cabang || !form.jumlah) { toast.error("Lengkapi data"); return; } toast.success(`Stok keluar −${form.jumlah} ${form.sparepart}`, { description: form.cabang }); onClose(); }} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white bg-red-600 hover:bg-red-700 active:scale-95 rounded-xl transition-all"><ArrowUpRight size={15} />Simpan</button>
